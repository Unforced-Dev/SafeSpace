...
export const PropertyDetails: React.FC<PropertyDetailsProps> = ({ property }) => {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Unresolved': return 'text-red-600 bg-red-50';
    ...
    <Card>
      <div className="space-y-6">
        <div>
          ...
          {property.landlord && (
            <p className="mt-1 text-gray-600">Landlord: {property.landlord}</p>
          )}
        </div>
              ...
              {property.violations.map((violation, index) => (
                <div key={index} className="border rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`ml-3 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getStatusColor(violation.status)}`}>
                      {violation.status}
                    </span>
                  </div>
                </div>
